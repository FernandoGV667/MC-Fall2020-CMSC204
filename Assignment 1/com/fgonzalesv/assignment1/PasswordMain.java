...
public class PasswordMain extends BorderPane {
  private Label passwordLabel, passwordALabel, instruction1Label, instruction2Label,
      instruction3Label, instruction4Label;
  ...
  public PasswordMain() {
    ...
    instruction6Label =
        new Label("\t5.  May not have more than 2 of the same character in sequence");
    ...
    try {
      ...
      if (PasswordCheckerUtility.isValidPassword(passwordString)) {
        if (PasswordCheckerUtility.isWeakPassword(passwordString)) {
          ...
          alert.showAndWait();
          // JOptionPane.showMessageDialog(null, "Password is OK but weak", "Password Status",
      ...
      alert.showAndWait();
      // JOptionPane.showMessageDialog(null, ex.getMessage(), "Password Status",
      // JOptionPane.PLAIN_MESSAGE);
    } catch (Exception ex) {
      alert.setContentText(ex.getMessage());
      alert.showAndWait();
      // JOptionPane.showMessageDialog(null, ex.getMessage(), "Password Error",
      // JOptionPane.PLAIN_MESSAGE);
    }
  }
  ...
  private class displayIllegalPasswords implements ActionListener {
    public void actionPerformed(ActionEvent theEvent) {
      ...
      alert.showAndWait();
      // JOptionPane.showMessageDialog(null, result, "Illegal Passwords",
      // JOptionPane.PLAIN_MESSAGE);
    }
  }
  ...
  public void readFile() {
    ...
    if (selectedFile != null) {
      ...
      try {
        ...
        alert.setContentText("Illegal Passwords\n" + results);
        alert.showAndWait();
        // JOptionPane.showMessageDialog(null, results, "Illegal Passwords",
        // JOptionPane.PLAIN_MESSAGE);
      } // end of try
      catch (FileNotFoundException ex) {
        alert.setContentText("File Error\n" + ex.toString());
        alert.showAndWait();
        // JOptionPane.showMessageDialog(null, ex.toString(), "File Error",
        // JOptionPane.PLAIN_MESSAGE);
        ex.printStackTrace();
      }
    }
